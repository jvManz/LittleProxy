...
public interface HttpFilters {
	/**
	 * Filters requests on their way from the client to the proxy. To interrupt
	 * processing of this request and return a response to the client
	 * immediately, return an HttpResponse here. Otherwise, return null to
	 * continue processing as usual.
	 * <p/>
	 * <b>Important:</b> When returning a response, you must include a mechanism
	 * to allow the client to determine the length of the message (see RFC 7230,
	 * section 3.3.3: https://tools.ietf.org/html/rfc7230#section-3.3.3 ). For
	 * messages that may contain a body, you may do this by setting the
	 * Transfer-Encoding to chunked, setting an appropriate Content-Length, or
	 * by adding a "Connection: close" header to the response (which will
	 * instruct LittleProxy to close the connection). If the short-circuit
	 * response contains body content, it is recommended that you return a
	 * FullHttpResponse.
	 * 
	 * @param httpObject
	...
	/**
	 * Filters requests on their way from the proxy to the server. To interrupt
	 * processing of this request and return a response to the client
	 * immediately, return an HttpResponse here. Otherwise, return null to
	 * continue processing as usual.
	 * <p/>
	...
	/**
	 * Informs filter that the HTTP request, including any content, has been
	 * sent.
	 */
	...
	/**
	 * Informs filter that a timeout occurred before the server response was
	 * received by the client. The timeout may have occurred while the client
	 * was sending the request, waiting for a response, or after the client
	 * started receiving a response (i.e. if the response from the server
	 * "stalls").
	 * 
	 * See {@link HttpProxyServerBootstrap#withIdleConnectionTimeout(int)} for
	 * information on setting the timeout.
	 */
	...
	/**
	 * Informs filter that proxy to server DNS resolution failed for the
	 * specified host and port.
	 * 
	 * @param hostAndPort
	 *            hostname and port the proxy failed to resolve
	 */
	 ...
	 * 
	 * @param serverCtx
	 *            the {@link io.netty.channel.ChannelHandlerContext} used to
