...
public class KeepAliveTest {
	...
	/**
	 * Tests that the proxy does not close the connection after a successful
	 * HTTP 1.1 GET request and response.
	 */
	@Test
	public void testHttp11DoesNotCloseConnectionByDefault() throws IOException, InterruptedException {
		mockServer.when(request().withMethod("GET").withPath("/success"), Times.exactly(2)).respond(
		...
		// construct the basic request: METHOD + URI + HTTP version + CRLF (to
	...
	/**
	 * Tests that the proxy keeps the connection to the client open after a
	 * server disconnect, even when the server is using connection closure to
	...
	public void testProxyKeepsClientConnectionOpenAfterServerDisconnect() throws IOException, InterruptedException {
		mockServer.when(request().withMethod("GET").withPath("/success"), Times.exactly(2)).respond(
				response()
						...
						.withConnectionOptions(
								new ConnectionOptions().withKeepAliveOverride(false).withSuppressContentLengthHeader(true)
		...
		for (int i = 1; i <= 2; i++) {
			...
			assertThat("Expected to receive an HTTP 200 from the server (iteration: " + i + ")", response, startsWith("HTTP/1.1 200 OK"));
			// the proxy will set the Transfer-Encoding to chunked since the
			// server is using connection closure to indicate the end of the
			// message.
			// (matching capitalized or lowercase Transfer-Encoding, since Netty
			// 4.1+ uses lowercase header names)
			assertThat("Expected proxy to set Transfer-Encoding to chunked", response.toLowerCase(Locale.US),
					containsString("transfer-encoding: chunked"));
			// the Transfer-Encoding is chunked, so the body text will be
			// followed by a 0 and 2 CRLFs
	...
	/**
	 * Tests that the proxy does not close the connection after a 502 Bad
	 * Gateway response.
	 */
	@Test
	public void testBadGatewayDoesNotCloseConnection() throws IOException, InterruptedException {
		mockServer.when(request().withMethod("GET").withPath("/success"), Times.exactly(1)).respond(
				response().withStatusCode(200).withBody("success"));
		...
		for (int i = 1; i <= 2; i++) {
			...
			assertThat("Expected to receive an HTTP 200 from the server (iteration: " + i + ")", response,
	...
	public void testGatewayTimeoutDoesNotCloseConnection() throws IOException, InterruptedException {
		...
		for (int i = 1; i <= 2; i++) {
			...
			assertThat("Expected to receive an HTTP 200 from the server (iteration: " + i + ")", response,
					startsWith("HTTP/1.1 504 Gateway Timeout"));
		}
	...
	public void testShortCircuitResponseDoesNotCloseConnectionByDefault() throws IOException, InterruptedException {
		mockServer.when(request().withMethod("GET").withPath("/success"), Times.exactly(1)).respond(
				response().withStatusCode(500).withBody("this response should never be sent"));
	...
	/**
	 * Tests that the proxy will close the connection after a short circuit
	 * response if the short circuit response contains a Connection: close
	 * header.
	 */
	@Test
	public void testShortCircuitResponseCanCloseConnection() throws IOException, InterruptedException {
		...
		// only send this request once, since we expect the short circuit
