...
public class HttpFilterTest {
	...
	/**
	 * Sets up the HttpProxyServer instance for a test. This method initializes
	 * the proxyServer and proxyPort method variables, and should be called
	 * before making any requests through the proxy server.
	 * 
	 * The proxy cannot be created in an @Before method because the
	 * filtersSource must be initialized by each test before the proxy is
	 * created.
	 * 
	 * @param filtersSource
	 *            HTTP filters source
	...
	public void testFiltering() throws Exception {
		...
		// sleep for a short amount of time, to allow the filter methods to be
		// invoked
		Thread.sleep(500);
		assertEquals("Response should have included the custom header from our pre filter", "1", response1.getFirstHeader("Header-Pre")
				.getValue());
		assertEquals("Response should have included the custom header from our post filter", "2", response1.getFirstHeader("Header-Post")
				.getValue());
		...
		// make sure we're getting the right remote address (and therefore the
		// right server channel context) in the
	...
	public void testConnectionFailedCalledAfterConnectionFailure() throws Exception {
		...
		assertTrue("Expected filter method to be called", filter.isClientToProxyRequestInvoked());
		// proxyToServerRequest is invoked before the connection is made, so it
		// should be hit
	...
	public void testFiltersAfterUnencryptedConnectionToUpstreamProxyFails() throws Exception {
		...
		// the server doesn't have to exist, since the connection to the chained
		// proxy will fail
	...
	/**
	 * Verifies the proper filters are invoked when an attempt to connect to an
	 * upstream chained proxy over SSL fails. (The
	...
	public void testFiltersAfterSSLConnectionToUpstreamProxyFails() throws Exception {
		// create an upstream chained proxy using the same SSL engine as the
		// chained proxy tests
	...
	public void testResponseTimedOutInvokedAfterServerTimeout() throws Exception {
		mockServer.when(request().withMethod("GET").withPath("/servertimeout"), Times.once()).respond(
				response().withStatusCode(200).withDelay(TimeUnit.SECONDS, 10).withBody("success"));
		...
		this.proxyServer = DefaultHttpProxyServer.bootstrap().withPort(0).withFiltersSource(filtersSource).withIdleConnectionTimeout(3)
				.start();

		org.apache.http.HttpResponse httpResponse = HttpClientUtil.performHttpGet("http://localhost:" + mockServerPort + "/servertimeout",
				proxyServer);
	...
	public void testRequestSentInvokedAfterLastHttpContentSent() throws Exception {
		...
		HttpFiltersSource filtersSource = new HttpFiltersSourceAdapter() {
			...
			public HttpFilters filterRequest(HttpRequest originalRequest) {
				return new HttpFiltersAdapter(originalRequest) {
					...
					@Override
					public void proxyToServerRequestSent() {
						// proxyToServerRequestSent should only be invoked after
						// the entire request, including payload, has been sent
						// to the server
						assertTrue(
		...
		// test with a POST request with a payload. post a large amount of data,
		// to force chunked content.
	...
	public void testNullHttpFilterSource() throws Exception {
		mockServer.when(request().withMethod("GET").withPath("/testNullHttpFilterSource"), Times.exactly(1)).respond(
		...
		org.apache.http.HttpResponse httpResponse = HttpClientUtil.performHttpGet("http://localhost:" + mockServerPort
				+ "/testNullHttpFilterSource", proxyServer);
		Thread.sleep(500);
	...
	private long now() {
		// using nanoseconds instead of milliseconds, since it is extremely
		// unlikely that any two callbacks would be invoked in the same
		// nanosecond,
