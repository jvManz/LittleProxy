...
public class DirectRequestTest {
	...
	public void testAnswerBadRequestInsteadOfEndlessLoop() throws Exception {
		...
		org.apache.http.HttpResponse response = HttpClientUtil.performHttpGet("http://127.0.0.1:" + proxyPort + "/directToProxy",
				proxyServer);
	...
	public void testAnswerFromFilterShouldBeServed() throws Exception {
		...
		int proxyPort = proxyServer.getListenAddress().getPort();
		org.apache.http.HttpResponse response = HttpClientUtil.performHttpGet("http://localhost:" + proxyPort + "/directToProxy",
				proxyServer);
		int statusCode = response.getStatusLine().getStatusCode();
	...
	public void testHttpsShouldCancelConnection() {
		...
		try {
			...
			assertThat("Expected an SSL exception when attempting to perform an HTTPS GET directly to the proxy", cause,
